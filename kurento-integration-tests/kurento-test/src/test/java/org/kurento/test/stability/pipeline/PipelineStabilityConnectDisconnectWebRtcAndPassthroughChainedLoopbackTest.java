 ...
 * Passthroughs to one WebRTC like a chain where the WebRTC is the first and the last element.<br/>
 *
 * <pre>
 * Pipeline -> WebRTCEndpoint -> PassThrough -> PassThrough -> ... -> PassThrough -> WebRTCEndpoint
 * </pre>
 *
 * Browser(s): <br>
 * · Chrome <br>
 ...
 * the WebRTC is the first and the last element</li>
 ...
 * and each connection launches 3 events (VIDEO, AUDIO and DATA). With the loopback, there is one
 * connection more</li>
 * <li>(M * (N + 1)) * 3 Disconnected Events are received. Each pipeline will have (N + 1)
 * disconnections, and each disconnection launches 3 events (VIDEO, AUDIO and DATA). With the
 * loopback, there is one connection more</li>
 * <li>FLOWING OUT Event is received</li>
 * <li>FLOWING IN Event is received</li>
...
public class PipelineStabilityConnectDisconnectWebRtcAndPassthroughChainedLoopbackTest
  ...
  @Ignore
  public void testConnectDisconnectWebRtcAndPassthroughChainedLoopbackOnePipelineDestroyPipeline()
      throws Exception {
  ...
  @Ignore
  public void testConnectDisconnectWebRtcAndPassthroughChainedLoopbackOnePipelineDestroyEachElement()
      throws Exception {
    // Stabilize the memory on the KMS
    doTest(1, true, false);
    // Use the assert for checking memory
  ...
  @Ignore
  public void testConnectDisconnectWebRtcAndPassthroughChainedLoopbackMPipelineDestroyPipeline()
      throws Exception {
    // Stabilize the memory on the KMS
  ...
  @Ignore
  public void testConnectDisconnectWebRtcAndPassthroughChainedLoopbackMPipelineDestroyEachElement()
      throws Exception {
    // Stabilize the memory on the KMS
    doTest(M, true, false);
    // Use the assert for checking memory
  ...
  private void doTest(int numPipelinesToCreate, boolean destroyEachElement, boolean checkMemory)
    ...
    for (int i = 1; i <= ITERATIONS; i++) {
      ...
      // With the loopback, there is one connection more
      ...
      for (int j = 0; j < numPipelinesToCreate; j++) {
        ...
        Assert.assertTrue("Not received FLOWING OUT event in webRtcRoot",
            flowingOutLatch.await(getPage(j).getTimeout(), TimeUnit.SECONDS));
        ...
        if (eachPassThroughChildren.size() > 0) {
          ...
          // Loopback
          eachPassThroughChildren.get(eachPassThroughChildren.size() - 1).connect(webRtcRoot);

          Assert.assertTrue("Not received FLOWING IN event in webRtcRoot",
              flowingInLatch.await(getPage(j).getTimeout(), TimeUnit.SECONDS));
        }
      ...
      for (Entry<MediaPipeline, ArrayList<PassThrough>> element : mediaPipelinePassThroughChildren
        ...
        if (eachPassThroughChildren.size() > 0) {
          ...
          eachPassThroughChildren.get(eachPassThroughChildren.size() - 1).disconnect(webRtcRoot);
        }
      }
